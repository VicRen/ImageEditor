...
import com.juphoon.imgeditor.core.PictureObj;
import com.juphoon.imgeditor.core.PictureMode;
import com.juphoon.imgeditor.core.PicturePath;
import com.juphoon.imgeditor.core.PictureText;
import com.juphoon.imgeditor.core.homing.PictureHomingAnimator;
import com.juphoon.imgeditor.core.homing.PictureHoming;
import com.juphoon.imgeditor.core.sticker.PictureSticker;
import com.juphoon.imgeditor.core.sticker.PictureStickerPortrait;
...
public class PictureView extends FrameLayout implements Runnable, ScaleGestureDetector.OnScaleGestureListener,
    ...
    private PictureObj mImage = new PictureObj();

    private GestureDetector mGDetector;

    private ScaleGestureDetector mSGDetector;

    private PictureHomingAnimator mHomingAnimator;
...

    {
        ...
        mDoodlePaint.setStrokeWidth(PicturePath.BASE_DOODLE_WIDTH);
        mDoodlePaint.setColor(Color.RED);
        mDoodlePaint.setPathEffect(new CornerPathEffect(PicturePath.BASE_DOODLE_WIDTH));
        ...
        mMosaicPaint.setStrokeWidth(PicturePath.BASE_MOSAIC_WIDTH);
        mMosaicPaint.setColor(Color.BLACK);
        mMosaicPaint.setPathEffect(new CornerPathEffect(PicturePath.BASE_MOSAIC_WIDTH));
    ...
    private void startHoming(PictureHoming sHoming, PictureHoming eHoming) {
        if (mHomingAnimator == null) {
            mHomingAnimator = new PictureHomingAnimator();
    ...
    private void onDrawImages(Canvas canvas) {
        ...
        if (mImage.getMode() == PictureMode.DOODLE && !mPen.isEmpty()) {
            mDoodlePaint.setColor(mPen.getColor());
            mDoodlePaint.setStrokeWidth(PicturePath.BASE_DOODLE_WIDTH * mImage.getScale());
            canvas.save();
            RectF frame = mImage.getClipFrame();
            canvas.rotate(-mImage.getRotate(), frame.centerX(), frame.centerY());
            canvas.translate(getScrollX(), getScrollY());
    ...
    public <V extends View & PictureSticker> void addStickerView(V stickerView, LayoutParams params) {
        if (stickerView != null) {
    ...
    public void addStickerText(PictureText text) {
        PictureStickerTextView textView = new PictureStickerTextView(getContext());
    ...
    boolean onInterceptTouch(MotionEvent event) {
        if (isHoming()) {
            stopHoming();
            return true;
        } else if (mImage.getMode() == PictureMode.CLIP) {
            return true;
        }
        return false;
    }
    ...
    private class MoveAdapter extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            return PictureView.this.onScroll(distanceX, distanceY);
        }
    ...
    private static class Pen extends PicturePath {
        ...
        PicturePath toPath() {
            return new PicturePath(new Path(this.path), getMode(), getColor(), getWidth());
        }
    }
}
